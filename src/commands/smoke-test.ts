...
export interface SmokeTestCommandOptions {
  product: string;
  apiBase?: string;
  token?: string;
  inputPath?: string;
...
export async function smokeTestCommand(
  opts: SmokeTestCommandOptions,
): Promise<void> {
  ...
  try {
    if (!opts.noPreflight) {
      await preflightCredits({
        client,
        required: MODULE_CREDIT_COSTS["smoke-test"]!,
    ...
    printSubmitResponse({
      job_id: data.job_id,
      status: data.status,
      module_type: data.module_type ?? "smoke-test",
