...
- **`resolveBillingUrl()` now appends `?channel=cli` to the billing URL** (default `https://diffmode.app/app/billing?channel=cli`) so out-of-credits users land on the CLI credit-packs view (Starter $20/10 cr · Builder $40/30 cr · Scale $99/100 cr). Relative `DIFFMODE_BILLING_URL` overrides are handled via the URL parser (correct query/fragment ordering, no duplicate `channel`).
